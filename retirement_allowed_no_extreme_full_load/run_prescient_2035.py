"""Run Prescient PCM simulation for 2035 (1HR commitment period config)."""

from prescient.simulator import Prescient


def _patch_prescient_curtailment_report():
    ...
    def _safe_report(ruc):
        ...
        for i, t in enumerate(time_periods):
            quantity = sum(
                _at_time(gd["p_max"], i) - _at_time(gd["pg"], i)
                for gd in rn_gens.values()
            )
            if quantity >= 5e-3:
                ...
                print(f"{t} {quantity:12.2f}")

    reporting.report_curtailment_for_deterministic_ruc = _safe_report
