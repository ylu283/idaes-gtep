"""GTEP driver with 4-hour commitment periods (6 per rep day).

Identical to driver_coal.py except:
- num_commit=6 (4-hour blocks instead of 1-hour)
- Hourly data (load, renewable p_max) averaged into 4-hour blocks
- min_up_time / min_down_time converted from hours to 4-hour periods
- commitmentPeriodLength patched from 1 to 4 hours
- Output saved to retirement_allowed_no_extreme_4hr_commit/
"""
...
# ── Configuration ─────────────────────────────────────────────────────────
BLOCK_HOURS = 4
NUM_COMMIT = 24 // BLOCK_HOURS  # 6
OUTPUT_DIR = "retirement_allowed_no_extreme_4hr_commit"
...
# ── Aggregate to 4-hour resolution ───────────────────────────────────────
...
try:
    mod_object.results = opt.solve(
        mod_object.model, tee=True, solver_options={"LogFile": "basic_logging_4hr.log"}
    )
except Exception as e:
    print(f"\nSolve failed: {e}")
    print("Writing model to LP file for IIS analysis...")
    mod_object.model.write("model_4hr.lp", io_options={"symbolic_solver_labels": True})
    print("Model written to model_4hr.lp")
    print("Computing IIS via gurobipy...")
    import gurobipy
    gm = gurobipy.read("model_4hr.lp")
    gm.computeIIS()
    gm.write("iis_4hr.ilp")
    print("IIS written to iis_4hr.ilp")
    n_constr = 0
